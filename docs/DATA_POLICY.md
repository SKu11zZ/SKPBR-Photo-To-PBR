...
The public v0.3 checkpoint contains the named tensors required by `StructuredReliefSpatialPBRNet.load_state_dict` plus bounded version, selection, and anti-cheating metadata. It is loaded through PyTorch's restricted `weights_only=True` path. It contains no optimizer state, sample identity, source path, local path, or target texture.

The two sheets under `examples/plane-d41` and the combined board under `examples/blind-f` were created from numeric procedural recipes after the corresponding weights were frozen. They are not members of the training or validation sets and contain no third-party material-library content. Larger internal reports and per-sample exports are deliberately excluded from the current public tree.
